...
class ArakoonClient :
    ...
    def _getConnection(self, nodeId):
        ...
        if connection is None:
            ...
            connection = ArakoonClientConnection ( nodeLocation )
...
if __name__ == "__main__" :
    ...
    client = ArakoonClient( )
    print "Talking to master %s" % client.whoMaster()

    r = "%d_%d" % ( random.randint(0, 1000000), random.randint(0, 1000000) )

    print "Found a server. Got back hello response : '%s'" % client.hello( "Haaai!")
    ...
    try:
        print client.get( "FAULTKEY" )
    except ArakoonNotFound, ex:
        print "Correctly got NotFound exception"

    print "Got back value '%s' for key '%s'" % (client.get( key ), key)

    client.delete( key )

    oldStartVal = value + "_start"
    client.set( "stars" , value + "_stars")
    client.set( "start" , oldStartVal )
    client.set( "stilo" , value + "_stilo")
    client.set( "stop" , value + "_stop")

    print "Got back elements from range query: "
    for key in client.range("start", True, "stop", False) :
        print " - %s " % key

    print "Got back elements from range entries query:"
    for key,value in client.range_entries( "start", True, "stop", False ) :
        print " - %s : %s" % (key,value )

    keyPrefix = 'st'
    print "Got back elements matching prefix '%s' " % keyPrefix
    for key in client.prefix( keyPrefix, -1 ) :
        print " - %s " % key
#

    print "Attempt to set key 'start' to value 'start'"

    newVal = client.testAndSet( "start" , oldStartVal , "start")
    if newVal != "start" :
        print "Attempt failed! Value is: '%s'" % newVal

    getVal = client.get ( "start" )
    if getVal != "start" :
        print "Found an issue. Value not set correctly. Got back value '%s'" % getVal
    else :
        print "Attempt successful."


    try :
        print "Attempt to set key 'start' to value 'start2'"
        value = client.testAndSet( "start" , oldStartVal, "start2")
        if value != "start" :
            print "Problemo. Updated value while it shouldn't have"
        else :
            print "Update correctly refused by server."
    except :
        print "Test and set correctly refused to update the value"


    if client.exists( "start") :
        print "Successfully validated existence of key 'start'"
    else :
        print "False positive on key existence"
    ...
    if not client.exists( "start") :
        print "Successfully validated non-existence of key 'start'"
    else :
        print "False negative on key existence"

    print "Attempt to do a sequence"
    client.set("zeloot","XXX")
    seq = Sequence()
    seq.addSet("zebra", "gestreept")
    seq.addSet("zen",   "cool")
    seq.addDelete("zeloot")
    client.sequence(seq)
    print client.get("zebra")
    print client.exists("zeloot")
