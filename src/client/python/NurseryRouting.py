...
class InternalRoutingNode(RoutingInfo):
    ...
    def getSmallestSeparator(self):
        '''
        go as far left as possible.
        The last separator you encounter is returned
        '''
        if not self._isInternal(self._left):
            return self._boundary
        else:
            s = self._left.getSmallestSeparator()
            return s

    def getLargestSeparator(self):
        if not self._isInternal(self._right):
            return self._boundary
        else:
            s = self._right.getLargestSeparator()
            return s

    def _isInternal(self, node):
        return node.__class__ == self.__class__

    def getSeparator(self,key):
        b = self._boundary
        if key < b :
            if self._isInternal(self._left):
                return self._left.separator(key)
            else:
                return b
        elif key > b :
            if self._isInternal(self._right):
                return self._right.separator(key)
            else:
                return b
