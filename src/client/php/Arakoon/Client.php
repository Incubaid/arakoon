...
class Arakoon_Client
{
	...
	private function getMasterIdFromNode($nodeId)
	{
		...
		try
		{
			$masterId = $this->sendMsgToNodeDecodeResult($nodeId, $message, "StringOption");
		}
		catch(Exception $exception)
		{
			Arakoon_Client_Logger::logDebug("Exception: $exception", __FILE__, __FUNCTION__, __LINE__);
	 ...
	 * @todo document
	 */
	private function determineMasterId()
	{
		Arakoon_Client_Logger::logTrace("Enter", __FILE__, __FUNCTION__, __LINE__);
		...
		while (count($nodes) > 0)
		{
			...
			$possibleMasterId = NULL;
			
			$possibleMasterId = $this->getMasterIdFromNode($randomNodeId);
			
			if ($possibleMasterId == NULL)
			{
				continue;
			}
			
			$this->_config->assertNodeExists($possibleMasterId);
			
			if (($randomNodeId == $possibleMasterId) || $this->isMaster($possibleMasterId))
			{
				Arakoon_Client_Logger::logInfo("$randomNodeId is master", __FILE__, __FUNCTION__, __LINE__);
			...
			{
				if ($nodes[$i]->getId() == $possibleMasterId)
				{
		...
		for ($i = 0; $i < $sendMsgTryCount; $i++)
		{
			...
			while ($currentTime < $deadLine)
			{
				...
				catch(Exception $exception)
				{
					...
					Arakoon_Client_Logger::logDebug("Exception: $exception", __FILE__, __FUNCTION__, __LINE__);
					Arakoon_Client_Logger::logWarning("Attempt " . $i + 1 . " to send a message to a node ($nodeId) failed", __FILE__, __FUNCTION__, __LINE__);
					
					if ($sendToMaster)
					{
						Arakoon_Client_Logger::logTrace("Reset master", __FILE__, __FUNCTION__, __LINE__);		
						$this->resetMaster();
					}			
				}
	...
	private function getNodeConnection($nodeId)
	{
		...
		if (!array_key_exists($nodeId, $this->_connections))
		{
			...
			$clusterId = $this->_config->getClusterId();
			$prologue = Arakoon_Client_Protocol::EncodePrologue($clusterId);
			
			$connection = new Arakoon_Client_NodeConnection($node, $prologue);
			
			$this->_connections[$nodeId] = $connection;
		}
		else
		{
			$connection = $this->_connections[$nodeId];
		}
