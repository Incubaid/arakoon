...
class Arakoon_Client_Logger
{
     ...
     * @param boolean 	$clearLogFile		clears the log file if true, defaults to false
     */
    public static function setup($logFileLocation, $level = self::DEFAULT_LOG_LEVEL, $clearLogFile = FALSE)
    {
    	self::setLogFileLocation($logFileLocation);
    	self::setLevel($level);    	
    	if ($clearLogFile)
    	{
    		self::clearLogFile($logFileLocation);
    	}
    }
	...
	*/
	...
	/**
	 * Clears the logger its log file.
	 *
	 * @return void
	 */
	public static function clearLogFile()
	{
		...
		{
			throw new Exception("Could not clear log file (exception: $exception)");
		}
	}
