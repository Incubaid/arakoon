...
let __client_server_wrapper__ cluster (real_test:real_test) =
  ...
  let server () = 
    ...
    let messaging = new Tcp_messaging.tcp_messaging 
      ("127.0.0.1", 7778) _CLUSTER drop_it
    in
    let disp     = MyDispatcher.create messaging store timeout_q resyncs in
    let driver   = MyDriver.create disp timeout_q (PQ.create()) in
...
let test_ping () =
  let real_test client = 
    ...
    OUnit.assert_equal ~printer:(fun s -> s) server_version "test_backend.0.0.0";
    Lwt.return ()
  in 
