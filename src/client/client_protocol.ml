...
let read_command (ic,oc) =
  ...
  begin
	...
	try
	  let c = Hashtbl.find int2code as_int32 in
          Lwt.return c
	with Not_found ->
...
let handle_exception oc exn= 
  ...
  | XException(Arakoon_exc.E_NOT_FOUND, msg) -> 
      Arakoon_exc.E_NOT_FOUND,msg, false, false
  ...
  | _ -> Arakoon_exc.E_UNKNOWN_FAILURE, "unknown failure", false, false
  in 
...
let protocol backend connection =
  ...
  let rec loop () =
    ...
    if closed 
    then
      Lwt_log.debug "leaving client loop" >>= fun () -> Lwt.return ()
    else
      begin
        Lwt_io.flush oc >>= fun() ->
        loop ()
