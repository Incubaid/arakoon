  ...
  read_command (ic,oc) >>= function
    | PING ->
        begin
          ...
          log_f "connection=%Lu PING: client_id=%S cluster_id=%S" id client_id cluster_id >>= fun () ->
    ...
    | EXISTS ->
	  ...
	  Llio.input_string ic >>= fun key ->
          log_f "connection=%Lu EXISTS: allow_dirty=%B key=%S" id allow_dirty key >>= fun () ->
	  Lwt.catch
    ...
    | GET ->
	  ...
	  Llio.input_bool   ic >>= fun allow_dirty ->
          Llio.input_string ic >>= fun  key ->
          log_f "connection=%Lu GET: allow_dirty=%B key=%S" id allow_dirty key >>= fun () ->
	  Lwt.catch
    ...
    | ASSERT ->
	  ...
	  Llio.input_string_option ic >>= fun vo ->
          log_f "connection=%Lu ASSERT: allow_dirty=%B key=%S" id allow_dirty key >>= fun () ->
	  Lwt.catch
    ...
    | ASSERTEXISTS ->
	  ...
	  Llio.input_bool ic          >>= fun allow_dirty ->
	  Llio.input_string ic        >>= fun key ->
          log_f "connection=%Lu ASSERTEXISTS: allow_dirty=%B key=%S" id allow_dirty key >>= fun () ->
	  Lwt.catch
	...
	begin
          ...
          log_f "connection=%Lu SET: key=%S" id key >>= fun () ->
	  Lwt.catch
	...
	begin
          Llio.input_string ic >>= fun key ->
          log_f "connection=%Lu DELETE: key=%S" id key >>= fun () ->
          Lwt.catch
	    (fun () ->
    ...
    | RANGE ->
	  ...
	  Llio.input_bool ic >>= fun allow_dirty ->
          ...
          log_f "connection=%Lu RANGE: allow_dirty=%B first=%s finc=%B last=%s linc=%B max=%i"
    ...
    | RANGE_ENTRIES ->
	  ...
	  Llio.input_bool          ic >>= fun linc  ->
	  Llio.input_int           ic >>= fun max   ->
          log_f "connection=%Lu RANGE_ENTRIES: allow_dirty=%B first=%s finc=%B last=%s linc=%B max=%i"
            id allow_dirty (p_option first) finc (p_option last) linc max >>= fun () ->
          Lwt.catch
	    (fun () ->
    ...
    | REV_RANGE_ENTRIES ->
	  ...
	  Llio.input_bool          ic >>= fun linc  ->
	  Llio.input_int           ic >>= fun max   ->
          log_f "connection=%Lu REV_RANGE_ENTRIES: allow_dirty=%B first=%s finc=%B last=%s linc=%B max=%i"
            id allow_dirty (p_option first) finc (p_option last) linc max >>= fun () ->
          Lwt.catch
	    (fun () ->
    ...
    | LAST_ENTRIES ->
	  ...
	  Sn.input_sn ic >>= fun i ->
          log_f "connection=%Lu LAST_ENTRIES: i=%Li" id i >>= fun () ->
	  Llio.output_int32 oc 0l >>= fun () ->
    ...
    | WHO_MASTER ->
        begin
          log_f "connection=%Lu WHO_MASTER" id >>= fun () ->
    ...
    | EXPECT_PROGRESS_POSSIBLE ->
        begin
          log_f "connection=%Lu EXPECT_PROGRESS_POSSIBLE" id >>= fun () ->
    ...
    | TEST_AND_SET ->
	  ...
	  Llio.input_string_option ic >>= fun expected ->
          ...
          log_f "connection=%Lu TEST_AND_SET: key=%S" id key >>= fun () ->
    ...
    | USER_FUNCTION ->
	  ...
	  Llio.input_string_option ic >>= fun po ->
          log_f "connection=%Lu USER_FUNCTION: name=%S" id name
          >>= fun () ->
	  Lwt.catch
	    (fun () ->
	      begin
    ...
    | PREFIX_KEYS ->
	  ...
	  Llio.input_string ic >>= fun key ->
	  Llio.input_int    ic >>= fun max ->
          log_f "connection=%Lu PREFIX_KEYS: allow_dirty=%B key=%S max=%i" id allow_dirty key max
          >>= fun () ->
    ...
    | MULTI_GET ->
	  ...
	  loop [] length >>= fun keys ->
          log_f "connection=%Lu MULTI_GET: allow_dirty=%B length=%i keys=%S" id allow_dirty length (String.concat ";" keys) >>= fun () ->
	  Lwt.catch
	    (fun () ->
    ...
    | SEQUENCE ->
        log_f "connection=%Lu SEQUENCE" id >>= fun () ->
        handle_sequence ~sync:false ic oc backend
    | SYNCED_SEQUENCE ->
        log_f "connection=%Lu SYNCED_SEQUENCE" id >>= fun () ->
        ...
        begin
          Lwt.catch(
            fun () ->
              ...
              log_f "connection=%Lu MIGRATE_RANGE" id
              >>= fun () ->
              decode_sequence ic >>= fun updates ->
    ...
    | STATISTICS ->
        begin
          log_f "connection=%Lu STATISTICS" id
          >>= fun () ->
    ...
    | COLLAPSE_TLOGS ->
	  ...
	  Llio.input_int ic >>= fun n ->
          log_f "connection=%Lu COLLAPSE_TLOGS: n=%i" id n >>= fun () ->
          Lwt.catch
	    (fun () ->
    ...
    | SET_INTERVAL ->
	    ...
	    (fun () ->
              Interval.input_interval ic >>= fun interval ->
              log_f "connection=%Lu SET_INTERVAL: interval %S" id (Interval.to_string interval) >>= fun () ->
    ...
    | GET_INTERVAL ->
        begin
          Lwt.catch(
            fun() ->
              log_f "connection=%Lu GET_INTERVAL" id >>= fun () ->
    ...
    | GET_ROUTING ->
        Lwt.catch
	  (fun () ->
            log_f "connection=%Lu GET_ROUTING" id >>= fun () ->
    ...
    | SET_ROUTING ->
	  ...
	  Routing.input_routing ic >>= fun routing ->
          log_f "connection=%Lu SET_ROUTING" id >>= fun () ->
	  Lwt.catch
	    (fun () ->
        ...
        begin
          Lwt.catch(
            fun () ->
              ...
              log_f "connection=%Lu SET_ROUTING_DELTA: left=%S sep=%S right=%S" id left sep right >>= fun () ->
    ...
    | GET_KEY_COUNT ->
        begin
          Lwt.catch
            (fun() ->
              log_f "connection=%Lu GET_KEY_COUNT" id >>= fun () ->
    ...
    | GET_DB ->
        begin
          Lwt.catch
            (fun() ->
              log_f "connection=%Lu GET_DB" id >>= fun () ->
    ...
    | OPT_DB ->
        begin
          Lwt.catch
            ( fun () ->
              log_f "connection=%Lu OPT_DB" id >>= fun () ->
    ...
    | DEFRAG_DB ->
        begin
          Lwt.catch
            (fun () ->
              log_f "connection=%Lu DEFRAG_DB" id >>= fun () ->
	...
	begin
	  ...
	  Lwt.catch
	    (fun () ->
              log_f "connection=%Lu CONFIRM: key=%S" id key >>= fun () ->
    ...
    | GET_NURSERY_CFG ->
        begin
          Lwt.catch (
            fun () ->
              log_f "connection=%Lu GET_NURSERY_CFG" id >>= fun () ->
              backend # get_routing () >>= fun routing ->
        ...
        begin
          Lwt.catch (
            fun () ->
              ...
              log_f "connection=%Lu SET_NURSERY_CFG: cluster_id=%S" id cluster_id >>= fun () ->
	    ...
	    (fun () ->
              ...
              let direction =
                ...
                then
                  ...
                  Routing.LOWER_BOUND
              in
              log_f "connection=%Lu GET_FRINGE: boundary=%s dir=%s"
                id
                (p_option boundary)
        ...
        begin
          Lwt.catch
            ( fun () ->
              ...
              log_f "connection=%Lu DELETE_PREFIX %S" id prefix >>= fun () ->
    ...
    | VERSION ->
      begin
        log_f "connection=%Lu VERSION" id >>= fun () ->
        Llio.output_int oc 0 >>= fun () ->
    ...
    | DROP_MASTER ->
      begin
        log_f "connection=%Lu DROP_MASTER" id >>= fun () ->
        Lwt.catch
