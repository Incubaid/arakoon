...
let _get client max_n =
  let rec loop n = 
    if n = max_n 
    then Lwt.return ()
    else
      begin
	_progress n 10000 >>= fun () ->
	begin
...
let _fill_transactions client max_n t size=
...
let benchmark ?(size=10) client =
  ...
  (* last_entries () >>= fun () -> *)
  Lwt_io.printlf "going to do %i 'sets'" max_n >>= fun () ->
  let t0 = Unix.gettimeofday () in
  _fill client max_n sz >>= fun () ->
  let t1 = Unix.gettimeofday () in
  Lwt_io.printlf "\nfill %i took: %f" max_n (t1 -. t0) >>= fun () ->
  let t = 100 in
  Lwt_io.printlf "going to do %i 'sets' in transactions of size %i"  max_n t 
  >>= fun () ->
  _fill_transactions client max_n t sz >>= fun () ->
  let t2 = Unix.gettimeofday() in
  Lwt_io.printlf "\nfill_transactions %i took : %f" max_n (t2 -.t1) >>= fun() ->
  _get client max_n >>= fun () ->
