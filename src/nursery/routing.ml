...
module Routing = struct
  ...
  let rec get_lower_sep parent_sep c_id = function
      | Branch(_l, s, r) when r = (Cluster c_id) -> Some s
      | Branch(l, _s, _r) when l = (Cluster c_id) -> parent_sep
      | Branch (l, s, r) ->
  ...
  let rec get_upper_sep parent_sep c_id = function
      | Branch(l, s, _r) when l = (Cluster c_id) ->
        Some s
      | Branch(_l, _s, r) when r = (Cluster c_id) ->
        parent_sep
      | Branch(l, s, r) ->
  ...
  let remove t c_id =
    let rec inner_remove t c_id _other_side =
      match t with
        ...
        | Cluster _c -> failwith "I hate ocaml's comparison"
    in
  ...
  let change t left sep right =
    ...
    then
      ...
      let rec _walk = function
        ...
        | Branch(l,_s, r) when (get_right_most l) = left && (get_left_most r) = right ->
          Branch(l, sep, r)
        | Branch(l,s,r) ->
          if s < sep
  ...
  let next_cluster t cluster_id =
    let rec inner_next acc = function
      | Cluster _ -> None
      | Branch (l, _s, r) when l = (Cluster cluster_id) ->
        Some(get_left_most r)
      | Branch (_l, _s, r) when r = (Cluster cluster_id) ->
        begin
          match acc with
  ...
  let prev_cluster t cluster_id =
    let rec inner_prev acc = function
      | Cluster _ -> None
      | Branch(l, _s, r) when r = (Cluster cluster_id) ->
        Some (get_right_most l)
      | Branch(l, _s, _r) when l = (Cluster cluster_id) ->
        begin
          match acc with
