...
module NC = struct
  ...
  let get t key =
    let cn = NCFG.find_cluster t.rc key in
    let todo conn = Common.get conn ~allow_dirty:false key in
    _with_master_connection t cn todo
  ...
  let __migrate t sep finalize publish migration =
  ...
  let migrate t clu_left (sep: string) clu_right  =
    ...
    in
    __migrate t (Some sep) finalize publish (from_cn, to_cn, direction)
