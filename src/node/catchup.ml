...
let catchup_tlog (type s) ~stop other_configs ~cluster_id  mr_name ((module S : Store.STORE with type t = s),store,tlog_coll)
  =
...
let catchup ~stop me other_configs ~cluster_id ((_,_,tlog_coll) as dbt) mr_name =
  ...
  >>= fun () ->
  catchup_tlog ~stop other_configs ~cluster_id mr_name dbt >>= fun until_i ->
