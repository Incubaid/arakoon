...
let failing_value =
  let k2 = "key2"
  and v2 = "value2" in
  Value.create_client_value_zero [Update.Sequence [Update.Set(k2, v2);
...
let value_asserts =
  let k1 = "key1"
  and v1 = "value1" in
  [(Value.create_client_value_zero [Update.Set(k1,v1)] false,
    [assert_k_v k1 v1]);
   (failing_value,
...
let test_safe_insert_value_with_partial_value_update () =
  with_store "tsivwpvu" (fun store ->
      ...
      let paxos_value = Value.create_client_value_zero [u1;u2;u3] false in
