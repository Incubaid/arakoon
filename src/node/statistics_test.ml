...
let test_correctness0 () = 
  let t0 = create_time_stats() in
  let () = update_time_stats t0 1.0 in
  let () = Printf.printf "t0=%s\n" (time_stats_to_string t0) in
...
let test_correctness1 () = 
  let t0 = create_time_stats() in
  let () = List.iter (update_time_stats t0) [1.0;0.9;1.1;1.0;0.8;1.2] in
  let () = Printf.printf "t0=%s\n" (time_stats_to_string t0) in
  OUnit.assert_bool "avg" (t0.avg ==:  1.);
